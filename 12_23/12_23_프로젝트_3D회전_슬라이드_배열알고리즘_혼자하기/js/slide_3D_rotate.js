;(function(window,document,$,undefined){
    ...
    function main3dslideFn(){
        ...
        $(".slide").eq(slide3d[0]).css({left:-300}).stop().fadeIn(0,function(){
            $(this).css({ zIndex:1, transform:"rotate(0) skew(0) perspective(4000px) scale(.3)" })
        });
        $(".slide").eq(slide3d[1]).css({left:-250}).stop().fadeIn(0,function(){
            $(this).css({ zIndex:2, transform:"rotate(0) skew(0) perspective(4000px) scale(.5)" })
        });
        $(".slide").eq(slide3d[2]).css({left:-150}).stop().fadeIn(0,function(){
            $(this).css({ zIndex:3, transform:"rotate(0) skew(0) perspective(7000px) scale(.8)" })
        });

        $(".slide").eq(slide3d[3]).css({left:0}).stop().fadeIn(0,function(){
            $(this).css({ zIndex:4, transform:"rotate(0) skew(0) perspective(9000px) scale(1.2)" })
        });
